...
public class HeatMap extends JPanel implements MouseListener {
    ...
    private double clickTemp = -1;

    public HeatMap() {
        // TODO: initialize the heat map, half cold, half hot
        for(int i=0; i<tempGrid.length; i++){
            for(int j=0; j<=tempGrid[i].length/2; j++) tempGrid[i][j]=-150;
            for(int j=tempGrid[i].length/2+1; j<tempGrid[i].length; j++) tempGrid[i][j]=150;
        }
    ...
    public int getRed(double temp) {
        ...
        if(temp<-74) return 0;
        return (int) (255-255/75.0*temp*-1);
    }

    public int getGreen(double temp) {
        if(temp>75) return (int) (255-255/75.0*(temp-76));
        if(temp<-75) return (int) (255-255/75.0*(temp+75)*-1);
        return 255;
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        int blockHeight = getHeight() / tempGrid.length;
        int blockWidth = getWidth() / tempGrid[0].length;
        for (int r = 0; r < tempGrid.length; r++) {
            for (int c = 0; c < tempGrid[r].length; c++) {

                double tVal = tempGrid[r][c];
                g.setColor(new Color(Math.clamp(getRed(tVal), 0, 255), Math.clamp(getGreen(tVal),0, 255), Math.clamp(getBlue(tVal), 0, 255)));
    ...
    public void mousePressed(MouseEvent event) {
        ...
        int x = event.getX();
        int y = event.getY();
        clickedRow=(int) (50*(y/800.0));
        clickedCol=(int) (50*(x/1300.0));
    ...
    private class Listener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            double[][] tGrid=new double[tempGrid.length][tempGrid[0].length];
            // TODO: recalculate every element of the 2D array
            // update the temperature values in tempGrid
            for(int i=0; i<tempGrid.length; i++) for(int j=0; j<tempGrid[0].length; j++){
                int s=0, n=0;
            ...
            if(clickTemp!=-1) tempGrid[clickedRow][clickedCol]=clickTemp;
